...
public final class ArraySiftDownMain {
    ...
    private static <Integers> void heapify(Array<Integer> array, int top) {
        assert array != null : "Violation of: array is not null";
    ...
    private static void siftDown(Array<Integer> array, int top, int last) {
        ...
        int left = (top * 2) + 1;
        int right = (top * 2) + 2;
        int curr = top; //update according to comparison of children

        if (left <= last && (array.entry(left) < array.entry(curr))) {
            curr = left;
        }
        if (right <= last && (array.entry(right) < array.entry(curr))) {
            curr = right;
        }
        if (curr != top) {
            array.exchangeEntries(top, curr);
            //recursive call with update
            siftDown(array, curr, last);
        }
