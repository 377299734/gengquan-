...
public final class RSSReader {
    ...
    private static void outputHeader(XMLTree channel, SimpleWriter out) {
        ...
        // checks again if there is a title
    ...
    private static void processItem(XMLTree item, SimpleWriter out) {
        ...
        while (item.numberOfChildren() > count) {
            if (getChildElement(item.child(count), "pubDate") >= 0) {
                dateSave = count;
            }
            if (getChildElement(item.child(count), "source") >= 0) {
                sourceSave = count;
            }
            if (getChildElement(item.child(count), "title") >= 0) {
                titleSave = count;
            }
            if (getChildElement(item.child(count), "link") >= 0) {
                linkSave = count;
            }
            count++;
        }
    ...
    public static void main(String[] args) {
        ...
        // command to write to the new file

        SimpleWriter file = new SimpleWriter1L(fileName);

        // user enters a url to a page

        out.print("Enter the URL of an RSS 2.0 news feed: ");
        String url = in.nextLine();

// the variable xml is assigned the value of the user entered url

        XMLTree xml = new XMLTree1(url);

        // checks to see if it a valid xml page

        while (!xml.label().equals("rss")
                || !xml.attributeValue("version").equals("2.0")) {
            out.print("XML is not formatted correctly.");

            out.print("Enter the URL of an RSS 2.0 news feed: ");
            url = in.nextLine();
        }

        // channel node assigned the value of the first child of the xml root
        ...
        // checks if the node is a valid one to be called

        int count = 0;
        while (channel.numberOfChildren() > count) {
            if (getChildElement(channel.child(count), "item") >= 0) {
                processItem(channel.child(count), file);
            }
            count++;
        }
