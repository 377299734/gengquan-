...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final void add(K key, V value) {
        ...
        Map<K, V> result = this.hashTable.entry(bucket);
    ...
    public final Pair<K, V> remove(K key) {
        ...
        int bucket = mod(key.hashCode(), this.hashTable.length());
        Map.Pair<K, V> result = this.hashTable.entry(bucket).remove(key);
        this.size--;
        return result;
    ...
    public final Pair<K, V> removeAny() {
        ...
        int i = 0;
        Map<K, V> bucket = this.hashTable.entry(i);
        Map<K, V> empty = new Map1L<K, V>();
        while (!contains) {
            bucket = this.hashTable.entry(i);
            if (!bucket.equals(empty)) {
                contains = true;
            }
            i++;
        }
        Map.Pair<K, V> result = bucket.removeAny();
        this.size--;
        return result;
    }
    ...
    public final boolean hasKey(K key) {
        ...
        if (this.hashTable.mayBeExamined(bucket)) {
            contains = this.hashTable.entry(bucket).hasKey(key);
        }
