...
public final class ExpressionEvaluator {
    ...
    private static int valueOfDigit(StringBuilder source) {
        ...
        while (source.length() > 0) {
            ...
            if (Character.isDigit(op)) {
                value = Character.digit(op, RADIX);
                StringBuilder rest = source.deleteCharAt(0);

                value += valueOfDigit(rest);
            } else {
                StringBuilder rest = source.deleteCharAt(0);
                value += valueOfDigit(rest);
            }
    ...
    private static int valueOfFactor(StringBuilder source) {
        ...
        if (source.charAt(0) == '(') {
            ...
            String number = source.substring(0, 0);
            source.deleteCharAt(0);
    ...
    private static int valueOfTerm(StringBuilder source) {
        ...
        int value = valueOfDigitSeq(source);
