...
public class SortingMachine5<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        int left = (top * 2) + 1;
        int right = (top * 2) + 2;
        int current = top; //update according to comparison of children

        if (left <= last
                && order.compare(array.entry(left), array.entry(current)) < 0) {
            current = left;
        }
        if (right <= last
                && order.compare(array.entry(right), array.entry(current)) < 0) {
            current = right;
        }
        if (current != top) {
            array.exchangeEntries(top, current);
            //recursive call with update
            siftDown(array, current, last, order);
        }
    ...
    private static <T> void heapify(Array<T> array, int top, Comparator<T> order) {
        ...
        if (array.length() > 1) {
            ...
            if (left < size && !isHeap(array, left, size - 1, order)) {
                heapify(array, left, order);
            }

            if (right < size && !isHeap(array, right, size - 1, order)) {
                heapify(array, right, order);
            }

            siftDown(array, current, size - 1, order);
        }
    ...
    public final void changeToExtractionMode() {
        ...
        int heapSize = this.entries.length();
    ...
    public final T removeFirst() {
        ...
        Array<T> removed = new Array1L<>(this.heapSize - 1);
        T entry = this.heap.entry(0);
        this.heapSize--;

        for (int i = 0; i < this.heapSize; i++) {
            removed.setEntry(i, this.heap.entry(i + 1));
        }

        this.heap.clear();

        this.heap.transferFrom(removed);

        heapify(this.heap, 0, this.machineOrder);

        assert this.conventionHolds();
        // Fix this line to return the result after checking the convention.
        //return this.heap.entry(this.heapSize); //"first" element;
