...
public final class Tokenizer {
    ...
    private static String nextWordOrSeparator(String text, int position) {
        ...
        String next = "";
        String charAtPos = text.substring(position, position + 1);
        int i = position;

        if (!SEPARATORS.contains(charAtPos)) {
    ...
    public static Queue<String> tokens(SimpleReader in) {
        ...
        while (!in.atEOS()) {
            ...
            while (position < text.length()) {
                ...
                String word = nextWordOrSeparator(text, position);

                if (!SEPARATORS.contains(word.substring(0, 1))) {
                    token.enqueue(word);
                }
