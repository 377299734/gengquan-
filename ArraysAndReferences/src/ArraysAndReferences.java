...
public final class ArraysAndReferences {
    ...
    private static NaturalNumber productOfArrayElements(NaturalNumber[] nnArray) {
        ...
        for (count = 0; count < nnArray.length; count++) {
            total.multiply(nnArray[count]);
        }
    ...
    private static void computePartialProducts(NaturalNumber[] nnArray) {
        ...
        int counter = 0;
        NaturalNumber total = new NaturalNumber2(1);
        for (counter = 0; counter < nnArray.length; counter++) {
            total.multiply(nnArray[counter]);

            NaturalNumber different = new NaturalNumber2(0);
            different.copyFrom(total);
            nnArray[counter] = different;
        }
