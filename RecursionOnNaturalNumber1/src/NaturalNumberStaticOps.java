...
public final class NaturalNumberStaticOps {
    ...
    private static String toStringWithCommas(NaturalNumber n) {
        ...
        String naturalString = n.toString();
        String newNaturalString = "";

        if (naturalString.length() > 3) {
            int i = naturalString.length() - 1;
            int j = naturalString.length() - 3;
            while (i >= 0) {
                if (j - i == 0) {
                    if (j == 0) {
                        newNaturalString = naturalString.charAt(i)
                                + newNaturalString;
                    } else {
                        newNaturalString = "," + naturalString.charAt(i)
                                + newNaturalString;
