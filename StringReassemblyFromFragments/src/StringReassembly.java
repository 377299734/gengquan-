...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        assert 0 <= overlap
                ...
                String next = str1.substring(0, str1.length() - overlap);
                String phrase = next + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet, String str) {
        ...
        while (strSet.size() > 0) {
            String possible = strSet.removeAny();
            if (str.indexOf(possible) < 0 && possible.indexOf(str) < 0) {
                count++;
            }
            temp.add(possible);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> text = new Set1L<>();
        while (!input.atEOS()) {
            String str = input.nextLine();
            if (text.size() == 0) {
                text.add(str);
            }
            addToSetAvoidingSubstrings(text, str);
        }
